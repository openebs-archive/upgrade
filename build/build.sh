...
# Get the git commit
if [ -f $GOPATH/src/github.com/openebs/upgrade/GITCOMMIT ];
then
    GIT_COMMIT="$(cat $GOPATH/src/github.com/openebs/upgrade/GITCOMMIT)"
else
...
env GOOS=$GOOS GOARCH=$GOARCH CGO_ENABLED=0  go build ${BUILD_TAG} -ldflags \
    "-X github.com/openebs/upgrade/pkg/version.GitCommit=${GIT_COMMIT} \
    -X main.CtlName='${CTLNAME}' \
    -X github.com/openebs/upgrade/pkg/version.Version=${VERSION}"\
    -o $output_name \
    ./cmd/${CTLNAME}
